...
import (
	...
	"github.com/pangum/http/internal/internal"
...
func NewClient(loader *pangu.Config, logger logging.Logger) (client *Client, err error) {
	wrapper := new(core.Wrapper)
	if err = loader.Load(wrapper); nil != err {
		return
	}

	conf := wrapper.Http.Client
	client = new(Client)
	client.Client = resty.New()
	client.logger = logger
	client.proxies = make(map[string]*config.Proxy)
	...
	if nil != conf.Proxy && *conf.Proxy.Enabled && "" == conf.Proxy.Target && 0 == len(conf.Proxies) {
		...
		if nil != conf.Proxy && *conf.Proxy.Enabled {
			target := gox.Ift("" == conf.Proxy.Target, internal.TargetDefault, conf.Proxy.Target)
...
func (c *Client) log(_ *resty.Client, req *http.Request) (err error) {
	...
	if nil != req.Body {
		if body, re := io.ReadAll(req.Body); nil != re {
			...
			req.Body = core.NopCloser{Reader: bytes.NewBuffer(body)}
...
func (c *Client) setProxy(client *resty.Client, req *resty.Request) (err error) {
	...
	} else if dp, defaultOk := c.proxies[internal.TargetDefault]; defaultOk {
		addr := dp.Addr()
		client.SetProxy(addr)
		c.logger.Debug("设置代理服务器", field.New("url", req.URL), field.New("Proxy", addr))
	}
